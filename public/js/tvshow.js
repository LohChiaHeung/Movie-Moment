//https://api.themoviedb.org/3/discover/tv?sort_by=popularity.desc&api_key=1cf50e6248dc270629e802686245c2c8
...
function showMovies(data) {
    ...
    data.forEach(movie => {
        const { title, poster_path, vote_average, overview, id, name } = movie;
...
function getMovieDetailss(id) {
    fetch(BASE_URL + '/tv/' + id + '?' + API_KEY + '&append_to_response=videos,credits,similar,release_dates').then(res => res.json())
        //https://api.themoviedb.org/3/movie/508?api_key=1cf50e6248dc270629e802686245c2c8
        .then(movieData => {
            console.log(movieData);
            closeNav();
